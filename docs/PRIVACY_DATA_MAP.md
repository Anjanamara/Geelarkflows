...
> **Document Version**: 1.1.0 (Production Verified Baseline)  
...
| **Request Type** (`request_type`) | Scoping service category (`flow`, `script`, `consulting`, `other`) | Customer selection | Cloudflare D1 (`custom_automation_requests`) | Resend (internal alert) | Persists in D1 | **Required** | None. |
...
| **Attachment Metadata & Storage** (`filename`, `size_bytes`, `storage_reference`) | Processing customer-submitted logs or screenshots | Inbound email attachments | Cloudflare D1 & Cloudflare R2 | Resend, Cloudflare R2 | Persists in D1/R2 | **Required** | Define attachment deletion policy (e.g. purge after 90 days). |
...
| **Password Verifier** (`password_hash`) | Admin authentication | PBKDF2-SHA256 (100k iter, random salt) | Cloudflare D1 (`admin_users`) | None | Retained with user | Internal | None. |
| **Admin Sessions** (`token_hash`, `user_agent`, `ip_address`) | Session authentication & security monitoring. **Stores raw administrator IP and user-agent.** | Admin login request | Cloudflare D1 (`admin_sessions`) | None | Auto-expires in 7 days (`expires_at`) | Internal | None. |
...
| `geelark_active_payment` | `localStorage` | Active order snapshot (`orderId`, `paymentId`, `payAddress`, `currency`, `totalUsd`, `status`) | Keeps payment screen and QR code accessible if customer refreshes during crypto transfer | Cleared on return to marketplace or order dismissal | **Required** (Functional / essential storage) | None. |
| `gf_admin_session` | `Cookie` (`HttpOnly`, `Path=/`, `SameSite=Lax`, `Max-Age=604800`, `Secure` on HTTPS) | 32-byte cryptographically random token | Admin authentication for `/admin` management panel | 7 days (`Max-Age=604800`) | Internal (Admin only) | None. |
...
3. **Owner Decisions Pending**:
   - Explicit operational retention durations for completed orders, closed custom request leads, and archived support mail.
   - Dedicated contact address for privacy-specific requests (e.g. `privacy@geelarkflows.com` vs `support@geelarkflows.com`).
